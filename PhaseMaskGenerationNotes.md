...
2. By default the source property in the slm object does not exist. In the 'SpotsFF_PhasesAndIntensities_v2' function I use the argument AMPslm = cp.array(fs_setup.slm.source["amplitude"]) to define the padded SLM phase mask
   zeroMatGPS[rDimOff:-rDimOff,rDimOff:-rDimOff] =\
        cp.multiply(AMPslm, cp.exp(1j*PHIslm)) # Padded matrix with SLM phase.
I will remove this reference by
